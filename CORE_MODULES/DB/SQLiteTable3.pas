...
type
  ...
  public
    ...
    property DB: TSQLiteDB_ptr read fDB; //Its My
  published
...
begin
  ...
  self.fInTrans := False;
  Self.fFileName := copy(FileName,1,MaxInt);
...
begin
  ...
  try
    iResult := SQLite3_Open16(PWideChar(fFileName), Fdb);
    ...
    case Bindings[I].VType of
      ...
      begin
        case Bindings[I].VType of
          ...
          vtPWideChar: begin
            DataPtr := PWideChar(Bindings[I].VPWideChar);
            DataSize := -1;
          end;
          vtWideString: begin
            DataPtr := PWideChar((@Bindings[I].VWideString)^);
            DataSize := -1;
          end;
          vtChar: begin
          ...
          vtWideChar: begin
            DataPtr := PWideChar(Bindings[I].VWideChar);
            DataSize := -1;
          end;
          else
      ...
      begin
        ...
        begin
          ...
          if (sqlite3_bind_blob(stmt, I+1, DataPtr, DataSize, tsqlite3destructor(@DisposePointer)) <> SQLITE_OK) then
            RaiseError('Could not bind BLOB', 'BindData');
        end
        else             
...
var
  Stmt: TSQLiteStmt;
  NextSQLStatement: PWideChar;
  iStepResult: integer;
begin
  try
    if SQLite3_Prepare16_v2(self.fDB, PWideChar(SQL), -1, Stmt, NextSQLStatement) <>
      SQLITE_OK then
...
begin
  ...
  if Sqlite3_Prepare16(self.fDB, PWideChar(SQL), -1, Stmt, NextSQLStatement) <>
    SQLITE_OK then
    RaiseError('Error executing SQL', SQL)
  else
...
begin
  if Assigned(Query.Statement) then
    sqlite3_bind_text16(Query.Statement, Index, PWideChar(Value), Length(Value), Pointer(SQLITE_STATIC))
  else
...
begin
  ...
  try
    ...
    iBindResult := SQLite3_Bind_Blob(stmt, 1, ptr, iSize, tsqlite3destructor(@DisposePointer));
...
begin
  {$IFDEF WIN32}
  sqlite3_create_collation(fdb, 'SYSTEM', SQLITE_UTF16LE, nil, tcollateXcompare(@SystemCollate));
  {$ENDIF}
end;
    ...
    begin
      ...
      begin
        case par.valuetype of
          ...
          SQLITE_TEXT:
            sqlite3_bind_text16(Stmt, i, PWideChar(par.valuedata),
...
begin
  ...
  try
    ...
    if SQLite3_Prepare16_v2(DB.fDB, PWideChar(SQL), -1, Stmt, NextSQLStatement) <> SQLITE_OK then
              ...
              if (ActualColType = SQLITE_NULL) then
                ...
                begin
                  ...
                  begin
                    ...
                    begin
                      new(thisstringvalue);
                      ptrValue := sqlite3_column_text16(stmt, i);
